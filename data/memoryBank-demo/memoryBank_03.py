...
SHORT_TERM_CACHE_CAPACITY = 10 # <--- 新增: 短期缓存的容量
...
def initialize_memory_file(memory_path: str, user_name: str):
    """【已优化】初始化内存文件。如果文件不存在，则创建；如果文件存在但用户是新用户，则追加用户信息。"""
...
def load_and_index_memory(memory_file_path: str, user_name: str, embeddings_model: StableHuggingFaceEmbeddings, force_rebuild: bool = False):
    """【已修复】加载或构建FAISS索引，并修正查询函数以避免TypeError。同时为记忆条目添加唯一ID。"""
...
def rerank_documents(query: str, documents: List[Document], top_n: int = 3) -> List[Document]:
    """【新增】使用CrossEncoder对检索到的文档进行重排序，以提高相关性。"""
    ...
    try:
        ...
        # 这是一个简单的基于关键字的模拟重排，实际效果远不如CrossEncoder
...
def answer_question(user_question: str, 
                    long_term_db: FAISS, 
                    short_term_cache: ShortTermCache, # <--- 修改: 接收短期缓存对象
                    history: List[dict], 
                    user_name: str) -> Tuple[str, List[Document]]:
    """【最终融合版】集成了双层记忆检索、重排、去重和优化Prompt的问答函数。"""
...
4.  **简洁精炼**: 综合所有信息后，请提供核心要点。避免直接复述【背景资料】中的原文。

---
### 背景资料：长期与短期综合记忆库
这是从你的知识库(包括长期记忆和短期缓存)中检索到的、与当前问题最相关的历史资料：
{retrieved_context}
---
### 背景资料：关于用户的工作偏好
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n========================================\n欢迎, {user_name}！我是您的高级AI知识助手(LongMem思想融合版)。\n输入 'exit' 或 'quit' 或 '退出' 来结束会话。\n========================================")
